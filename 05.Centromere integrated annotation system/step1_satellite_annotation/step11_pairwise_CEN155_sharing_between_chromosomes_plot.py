...
def extract_chr(filename):
    """
    Extract the 'Chr' followed by two digits pattern from a filename using regular expressions.
    Example: 'AA_Ogla_hap1.sat.CEN155.Chr01' -> 'Chr01'
    """
    match = re.search(r'Chr(\d{2})', filename)
    if match:
        return match.group(0) # Returns the full matched string, e.g., "Chr01"
    return None

def summarize_sharing_ratios(input_file, output_file):
    """
    Read sharing ratio files, aggregate by chromosome pairs, and calculate the average sharing ratio.
    """
    # Use defaultdict to simplify code: automatically initializes a list [0.0, 0] when a key is first accessed.
    ...
    try:
        with open(input_file, 'r') as f_in:
            ...
            for line in f_in:
                # Use split() to handle multiple spaces or tabs
                ...
                try:
                    ...
                    print(f"Warning: Could not parse SharingRatio, skipping line -> {line.strip()}")
                    continue

                chr1 = extract_chr(file1)
                chr2 = extract_chr(file2)

                # Ensure chromosome IDs are found in both filenames
                if chr1 and chr2:
                    # Create a canonical key by sorting to ensure (Chr01, Chr03) and (Chr03, Chr01) share the same key
                    canonical_key = tuple(sorted((chr1, chr2)))
                    
                    # Accumulate the sum of SharingRatio
                    summary_data[canonical_key][0] += ratio
                    # Accumulate the count of occurrences for this chromosome pair
    ...
    print("Data aggregation complete. Calculating averages and writing to output file...")
    ...
    with open(output_file, 'w') as f_out:
        ...
        for key in sorted_keys:
            ...
            # Calculate the average, avoiding division by zero errors
            average_ratio = total_ratio / count if count > 0 else 0.0
            
            # Write the result row, formatted to 4 decimal places
...
def plot_triangular_heatmap(df, figsize=(10, 8), cmap='Blues', 
    ...
    """
    Plot a triangular heatmap showing average sharing ratios between chromosomes.
    
    Parameters:
    -----------
    df : pandas.DataFrame
        DataFrame containing columns 'chrA', 'chrB', and 'average_sharing'.
    ...
    annot : bool, default True
        Whether to annotate numerical values in cells.
    cbar_label : str
    ...
    for _, row in df.iterrows():
        ...
        matrix[j, i] = row['average_sharing']  # Fill symmetrically
    
    # Create a mask to display only the upper triangle (including diagonal)
...
if __name__ == "__main__":
    ...
    # Input filename (the original file provided)
    input_filename = "sharing_results_pairwise.tsv" 
    
    # Output filename (the summary file to be generated)
