...
def classify_species(seq_id):
    """Layer 1: Classification by species/subspecies (Added 5SrDNA as an independent functional locus category)."""
    sid = seq_id.upper()
    ...
    if "CRM" in sid:
        return "CRM_species"
    if "5SRDNA" in sid:
        return "5SrDNA_locus"  # Independent category for the species/locus layer
    return "Other"


def classify_cen(seq_id):
    """Layer 2: Classification by CEN/CRM repeat type."""
    sid = seq_id.upper()
    if "CEN155" in sid:
        return "CEN155"
    if "CEN165" in sid:
...
def main():
    ...
    species_colors_map = {
        ...
        "GG":     "#9D5427",
        "CRM_species": "#595959",
        "5SrDNA_locus": "#DF46CA",  # Color for 5SrDNA in the species/locus layer
        "Other":       "#CCCCCC"
    }
    ...
    cen_colors_map = {
        ...
        "NUMT": "#64288B",
        "NUPT": "#4643CF",
        "5SrDNA": "#C6F35E",        # Bright green: 5SrDNA in the CEN layer
        "Other":   "#EEEEEE"
    }
    ...
    # === Add dual legends, standardizing label suffixes ===
    ...
    # Use figure-level legends to avoid overlapping
    fig = cg.fig
    fig.legend(
        handles=handles1,
