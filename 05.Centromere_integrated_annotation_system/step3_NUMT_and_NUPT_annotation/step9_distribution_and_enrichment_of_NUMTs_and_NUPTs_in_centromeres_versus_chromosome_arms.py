...
def prepare_data(df):
    ...
    # 2. Function for display name conversion
    def get_display_name(assembly):
        # If a direct match to the full name is found (e.g., AA_Osat_hap2)
        if assembly in label_map:
            return label_map[assembly]
        
        # Otherwise, match the prefix (e.g., AA_Oruf) and preserve haplotype information
        prefix = "_".join(assembly.split("_")[:2])
...
def plot_stacked_lengths(df, output_prefix):
    ...
    # Set X-axis, using italics for species names (via mathtext for local italics)
    # Assuming species names contain "O. " or similar; handled simply here, can be refined as needed
...
def plot_enrichment(df, output_prefix):
    # Sort by Enrichment Ratio in descending order to highlight contrast
    df_plot = df.sort_values('enrichment_ratio', ascending=False)
    
    plt.figure(figsize=(12, 10))
    sns.set_style("whitegrid")
    
    # Set aesthetically pleasing green and red colors
    ...
    # Display specific Ratio values at the end of bars
    for bar in bars:
        width = bar.get_width()
    ...
    # Limit X-axis range to prevent numerical labels from exceeding boundaries
