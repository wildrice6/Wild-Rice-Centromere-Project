--- START OF FILE Step5_script for centromere inference and visualization from single-sample or single-seg.txt ---
...
# Build OptionParser object for argument parsing
...
if (nzchar(known_cen_file) && file.exists(known_cen_file)) {
  ...
  if (inherits(raw_known, "error")) {
    ...
    if (any(is.na(c(filename_col, chr_col, start_col, end_col)))) {
      warning("⚠️  Known centromere file is missing essential columns (filename/Filename, Chr/Chromosome, Start-like, End-like). Skipping ground truth plotting.")
    } else {
      if (nzchar(known_cen_filename_filter)) {
        ...
        tmp <- raw_known
        warning("⚠️  known_cen_filename_filter is empty; using all records from the known centromere file.")
      }
      ...
      if (nrow(tmp) > 0) {
        if (!all(c(chr_col, start_col, end_col) %in% colnames(tmp))) {
          ...
          cat(sprintf("✅ Read %d known centromere records (after final filtering).\n", nrow(known_cen_df_for_plot)))
        }
      } else {
        cat(sprintf("⚠️  No matching records found for filename '%s' in known centromere file, or file is empty after filtering.\n", known_cen_filename_filter))
      }
    }
  }
} else {
  if (nzchar(known_cen_file) && !file.exists(known_cen_file)) {
    cat(sprintf("ℹ️  Known centromere file '%s' not found. Skipping.\n", known_cen_file))
  } else {
    cat("ℹ️  Path to known centromere file not provided. Skipping.\n")
  }
}

# Read segmented data
if (!file.exists(sample_file)) { stop(sprintf("❌ Input BDG file not found: %s", sample_file)) }
cat(sprintf("ℹ️  Reading segmented BDG: %s\n", sample_file))
dat_all <- fread(sample_file, col.names = c("chr","start","end","log2ratio"))
if (!nrow(dat_all)) stop("❌ Segmented BDG input file is empty or failed to load!")

# Read raw log2ratio data for plotting if provided
dat_raw_all <- NULL
if (nzchar(raw_log2_file_path)) {
  if (file.exists(raw_log2_file_path)) {
    cat(sprintf("ℹ️  Reading raw log2ratio BDG file: %s\n", raw_log2_file_path))
    dat_raw_all <- tryCatch(
      fread(raw_log2_file_path, col.names = c("chr","start","end","log2ratio_raw")),
      error = function(e) {
        warning(sprintf("⚠️  Failed to read raw log2ratio BDG file '%s': %s. Skipping raw data plotting.", raw_log2_file_path, e$message))
        return(NULL)
      }
    )
    if (!is.null(dat_raw_all) && !nrow(dat_raw_all)) {
      warning(sprintf("⚠️  Raw log2ratio BDG file '%s' is empty! Skipping raw data plotting.", raw_log2_file_path))
      dat_raw_all <- NULL
    }
  } else {
    warning(sprintf("⚠️  Provided raw log2ratio BDG file not found: %s. Skipping raw data plotting.", raw_log2_file_path))
  }
}


all_chromosomes_in_file <- sort(unique(dat_all$chr))
cat(sprintf("➡️  Chromosomes detected in segmented file: %s\n", paste(all_chromosomes_in_file, collapse = ", ")))
...
if (processed_user_spec_chr_str == "ALL" || processed_user_spec_chr_str == "") {
  ...
  cat(sprintf("➡️  Processing all chromosomes in the file (user specified '%s').\n", processed_user_spec_chr_str))
} else {
  ...
  if (length(requested_chromosomes) > 0) {
    ...
    if (length(missing_in_file) > 0) {
      cat(sprintf("⚠️  The following requested/default chromosomes were not found in the segmented input file and will be ignored: %s\n", paste(missing_in_file, collapse = ", ")))
    }
    if (length(chromosomes_to_process) > 0) {
      cat(sprintf("➡️  Processing the following specified/default chromosomes found in the segmented file: %s\n", paste(chromosomes_to_process, collapse = ", ")))
    } else {
      stop(sprintf("❌ All requested/default chromosomes (%s) were missing from the segmented input file, or the list is empty. Cannot proceed.", paste(requested_chromosomes, collapse = ", ")))
    }
  } else {
    stop(sprintf("❌ The --chromosomes parameter '%s' is invalid or empty after parsing. Please use a comma-separated list, 'ALL', or ''.", user_specified_chromosomes_str))
  }
}
if (length(chromosomes_to_process) == 0) { stop("❌ No chromosomes to process. Check input file and --chromosomes parameter.") }

inferred_cen_summary <- data.table()
avg_seg_summary      <- data.table()

for (current_chrom_seg in chromosomes_to_process) {
  cat(sprintf("\n===== Processing segmented chromosome %s =====\n", current_chrom_seg))

  chr_dat <- dat_all[chr == current_chrom_seg]
  if (!nrow(chr_dat)) {
    cat(sprintf("ℹ️  Chromosome %s has no data points in segmented file. Skipping.\n", current_chrom_seg))
    next
  }
  ...
  if (!is.null(dat_raw_all)) {
    ...
    cat(sprintf("ℹ️  Searching for chromosome '%s' in raw data (based on segmented chromosome '%s' and target haplotype suffix '%s')\n",
    ...
    if (nrow(chr_dat_raw) > 0) {
      ...
      cat(sprintf("✓ Found %d raw data records for %s.\n", nrow(chr_dat_raw), target_raw_chrom_name))
    } else {
      cat(sprintf("ℹ️  Chromosome '%s' has no data points in raw log2ratio data.\n", target_raw_chrom_name))
      chr_dat_raw <- NULL
    }
  }
    ...
    if (length(candidate_raw_block_indices) > 0) {
      ...
      if (multi_cen_min_raw_block_points > 0) {
        ...
        if (any(block_weights > 0)) {
          best_block_idx_in_candidates <- find_best_idx <- which.max(block_weights)
  ...
  for (cen_region in all_inferred_cens_for_chrom) {
    ...
    cat(sprintf("Inferred centromere region for %s: %d-%d (%.2f-%.2f Mb)\n",
  ...
  for (undo_sd in undo_sd_vec) {
    ...
    message(">>> Writing AvgFit PDF: ", pdf_avg)
...
if (nrow(inferred_cen_summary) > 0) {
  message(">>> Writing summary table: ", cen_out)
  fwrite(inferred_cen_summary, cen_out, sep = "\t")
} else {
  message("ℹ️  Inferred centromere summary is empty; file not written: ", cen_out)
}
if (nrow(avg_seg_summary) > 0) {
  message(">>> Writing summary table: ", avg_out)
  fwrite(avg_seg_summary,      avg_out, sep = "\t")
} else {
  message("ℹ️  Average region means summary is empty; file not written: ", avg_out)
}

cat(sprintf("\n✅ Analysis complete. Results saved to: %s\n", output_dir))
--- END OF FILE Step5_script for centromere inference and visualization from single-sample or single-seg.txt ---